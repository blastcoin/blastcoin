...
uint256 hashGenesisBlock("0xa6a752d6adab7c526e117cd2f586db6385918c3b5002b1a8f621ce1df09daf3f");
...
{
    // Blastcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Blastcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Blastcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Blastcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa6a752d6adab7c526e117cd2f586db6385918c3b5002b1a8f621ce1df09daf3f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1389848805;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1140470;
        ...
        printf("%d\n", block.nTime);
        printf("%d\n", block.nNonce);
        assert(block.hashMerkleRoot == uint256("0x8d3c37b0506fe0383d87c87fd37cbf0928e311ad7dc1ffb8b9b8553fd6cb0977"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Blastcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BlastcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BlastcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BlastcoinMiner(CWallet *pwallet)
{
    printf("BlastcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("blastcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BlastcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BlastcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BlastcoinMiner, pwallet));
}
